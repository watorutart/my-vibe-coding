...
describe('usePetProgress Hook', () => {
  ...
  describe('経験値獲得システム', () => {
    it('should calculate experience correctly from interactions', () => {
      const { result } = renderHook(() => usePetProgress(mockPet, mockOnUpdate))
  ...
  describe('レベルアップシステム', () => {
    it('should trigger level up when experience threshold is reached', () => {
      ...
      expect(mockOnUpdate).toHaveBeenCalled()
      const updatedPet = mockOnUpdate.mock.calls[mockOnUpdate.mock.calls.length - 1][0]
    ...
    it('should not exceed maximum level', () => {
      ...
      const { result } = renderHook(() => usePetProgress(maxLevelPet, mockOnUpdate))
  ...
  describe('レベル依存の必要経験値', () => {
    ...
    it('should calculate experience to next level correctly', () => {
      ...
      const { result } = renderHook(() => usePetProgress(midLevelPet, mockOnUpdate))
  ...
  describe('進行状況の取得', () => {
    it('should provide current progress information', () => {
      ...
      const { result } = renderHook(() => usePetProgress(experiencedPet, mockOnUpdate))
