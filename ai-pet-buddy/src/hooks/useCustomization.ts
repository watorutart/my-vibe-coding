...
export function useCustomization(
  ...
  const [previewCustomization, setPreviewCustomization] =
    useState<PetCustomization>(DEFAULT_CUSTOMIZATION);
  ...
  const updateName = useCallback(
    (name: string): NameValidationResult => {
      ...
      if (validation.isValid) {
        const baseCustomization = isPreviewMode
  ...
  const updateColor = useCallback(
    (color: string): ColorValidationResult => {
  ...
  const addAccessory = useCallback(
    (accessoryId: string): { success: boolean; error?: string } => {
      const validation = validateAccessoryId(
        accessoryId,
      ...
      const accessory = customizationState.available.find(
      ...
      const baseCustomization = isPreviewMode
        ? previewCustomization
        : customizationState.current;
      const updatedCustomization = addAccessoryToCustomization(
        baseCustomization,
        accessory
      );
  ...
  const removeAccessory = useCallback(
    (accessoryId: string) => {
      const baseCustomization = isPreviewMode
        ? previewCustomization
        : customizationState.current;
      const updatedCustomization = removeAccessoryFromCustomization(
        baseCustomization,
        accessoryId
      );
  ...
  const saveAsPreset = useCallback(
    (presetName: string): { success: boolean; error?: string } => {
  ...
  // アクセサリー解除
  const unlockAccessoryById = useCallback(
    (accessoryId: string) => {
      const updatedAccessories = unlockAccessory(
        accessoryId,
        customizationState.available
      );
