...
interface UseDataPersistenceReturn {
  loadInitialData: () => { pet: Pet | null; conversationHistory: ConversationMessage[] };
  saveData: (pet: Pet, conversationHistory: ConversationMessage[]) => Promise<void>;
  setupAutoSave: (pet: Pet, conversationHistory: ConversationMessage[]) => void;
  clearAutoSave: () => void;
  setupBeforeUnloadSave: (pet: Pet, conversationHistory: ConversationMessage[]) => void;
}
 ...
 * @returns データ永続化関数群
 */
export function useDataPersistence(options: UseDataPersistenceOptions = {}): UseDataPersistenceReturn {
  ...
  // データ保存（スロットリング付き）
  const saveData = useCallback(async (pet: Pet, conversationHistory: ConversationMessage[]) => {
    const now = Date.now();
  ...
  // 自動保存設定
  const setupAutoSave = useCallback((pet: Pet, conversationHistory: ConversationMessage[]) => {
    // 既存のタイマーをクリア
    ...
    const interval = options.autoSaveInterval || settings?.autoSaveInterval || 30000;
  ...
  const setupBeforeUnloadSave = useCallback((pet: Pet, conversationHistory: ConversationMessage[]) => {
    // 既存のリスナーをクリア
