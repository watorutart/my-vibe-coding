...
export function usePetProgress(
  ...
  const getProgressInfo = useCallback((): ProgressInfo => {
    ...
    const required = getRequiredExperience(currentLevel);
    const percentage =
      required > 0 ? (currentExperience / required) * 100 : 100;
  ...
  const applyLevelUpBonuses = useCallback(
    (newLevel: number, currentStats: Pet['stats']) => {
      ...
      return {
        ...
        happiness: Math.min(
          100,
          currentStats.happiness + bonusPerLevel.happiness
        ),
