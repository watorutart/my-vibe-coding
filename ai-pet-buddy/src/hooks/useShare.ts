...
export const useShare = (): UseShareReturn => {
  ...
  const [lastShareImageUrl, setLastShareImageUrl] = useState<string | null>(
    null
  );
   ...
   * SNSシェア実行
   */
  const performSocialShare = useCallback(
    async (options: SocialShareOptions): Promise<ShareResult> => {
      try {
        setIsSharing(true);
        setError(null);
   ...
   * 画像ダウンロード実行
   */
  const performDownloadImage = useCallback(
  ...
  const generateShareData = useCallback(
    (imageDataUrl: string, statsData?: StatsCardData): ShareData => {
   ...
   * 統計カード生成
   */
  const createStatsCard = useCallback(
    async (
      statsData: StatsCardData,
      config: ImageGenerationConfig = {}
    ): Promise<string> => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : '統計カードの生成に失敗しました';
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setIsSharing(false);
   ...
   * ウォーターマーク追加
   */
  const performAddWatermark = useCallback(
    async (
      imageDataUrl: string,
