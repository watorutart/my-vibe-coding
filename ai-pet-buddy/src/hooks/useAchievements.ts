...
export interface UseAchievementsReturn {
  ...
  getNextAchievements: (
    limit?: number
  ) => ReturnType<typeof getNextAchievements>;
...
export function useAchievements(
  ...
  const saveAchievements = useCallback(async (): Promise<void> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to save achievements';
  ...
  const handleStateUpdate = useCallback(
    (newState: AchievementState, petData: Pet): AchievementState => {
      const { updatedState, notifications: newNotifications } =
        checkAchievements(newState, petData);
    ...
    try {
      ...
      if (savedData) {
        ...
        if (parsed.badges && parsed.titles && parsed.progress) {
          ...
          console.warn(
            'Invalid achievement data found, initializing new state'
          );
  ...
  const recordLevelUp = useCallback(
    (updatedPet: Pet): void => {
      setAchievementState(currentState => {
        const newProgress = updateLevelProgress(
  ...
  const recordEvolution = useCallback(
    (timestamp: number = Date.now()): void => {
      setAchievementState(currentState => {
        const newProgress = updateEvolutionProgress(currentState.progress, {
          timestamp,
        });
        actionsCountRef.current += 1;
  ...
  const endSession = useCallback((): void => {
    if (opts.enableSessionTracking && sessionStartRef.current) {
      ...
      setAchievementState(currentState => {
        const newProgress = updateSessionProgress(
          currentState.progress,
          sessionData
        );
   ...
   */
  const getNextAchievementsCallback = useCallback(
    (limit: number = 3) => {
