...
function updatePetStats(
  pet: Pet,
  updates: Partial<{ happiness: number; energy: number; health: number }>
): Pet {
  const newHunger =
    updates.health !== undefined ? 100 - updates.health : pet.stats.hunger;
  return {
    ...pet,
    stats: {
      ...pet.stats,
...
export function getNextEvolutionStage(
  ...
  const currentStageId = progress
    ? progress.currentStage
    : getCurrentEvolutionStage(pet).id;
...
export function checkEvolutionRequirements(
  ...
  return {
    ...
    missingRequirements: {
      ...
      happiness: meetsHappiness
        ? 0
        : targetStage.requiredStats.happiness - stats.happiness,
...
export function calculateEvolutionProgress(
  ...
  const levelProgress =
    nextStage.requiredLevel > 0
      ? Math.min(100, (stats.level / nextStage.requiredLevel) * 100)
      : 100;

  const happinessProgress =
    nextStage.requiredStats.happiness > 0
      ? Math.min(
          100,
          (stats.happiness / nextStage.requiredStats.happiness) * 100
        )
      : 100;

  const energyProgress =
    nextStage.requiredStats.energy > 0
      ? Math.min(100, (stats.energy / nextStage.requiredStats.energy) * 100)
      : 100;

  const healthProgress =
    nextStage.requiredStats.health > 0
 ...
 */
export function syncProgressWithPet(
  pet: Pet,
  progress?: EvolutionProgress
): EvolutionProgress {
