...
describe('evolutionEngine', () => {
  ...
  describe('getCurrentEvolutionStage', () => {
    ...
    it('should return teen stage for level 15', () => {
      const pet = { ...mockBasePet, stats: { ...mockBasePet.stats, level: 15 } };
      const stage = getCurrentEvolutionStage(pet);
      expect(stage.id).toBe('teen');
    });

    it('should return adult stage for level 30', () => {
      const pet = { ...mockBasePet, stats: { ...mockBasePet.stats, level: 30 } };
      const stage = getCurrentEvolutionStage(pet);
      expect(stage.id).toBe('adult');
    });

    it('should return elder stage for level 50+', () => {
      const pet = { ...mockBasePet, stats: { ...mockBasePet.stats, level: 50 } };
      const stage = getCurrentEvolutionStage(pet);
  ...
  describe('evolvepet', () => {
    ...
    it('should apply correct bonuses during evolution', () => {
      ...
      expect(result.pet.stats.happiness).toBe(pet.stats.happiness + childStage.bonuses.happiness);
      expect(result.pet.stats.energy).toBe(pet.stats.energy + childStage.bonuses.energy);
      // health bonus applied as hunger reduction
      expect(result.pet.stats.hunger).toBe(pet.stats.hunger - childStage.bonuses.health);
    });
